...
def mentod():
    ...
    print(' %s[%s1%s] Method 1 Best(%sRecommended%s)'%(N,H,N,H,N))
#-------- LOADING ANIMASI ------------
def loading():
...
\033[93;1m Owner   :        Hassan
\033[93;1m Facebook:       Ahmad
\033[93;1m Github  :        Not Found
\033[93;1m Version :       -0.0
\x1b[1;96m----------------------------------------------- """)
#CRACK SELESAI
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        print(f'\n%s══════════════════════════════════════════\n [%s✓%s] %sCracking By Ahmad Usercrack...\n%s══════════════════════════════════════════'%(N,H,N,H,N))
        ...
        if cek_cp =="":
            ...
            if ww in ("Y","y","ya"):
                ubahP.append("y")
                print(f" {N}[{H}•{N}] Password example : {H}Ahmad123{N}")
            ...
            for memek in cp:
                ...
                titid  = kontol.split(' • ')
                print(f'{N}══════════════════════════════════════════\n {H}LOGIN PROCESS')
                jalan(f' {N}[{M}?{N}] Account : {K}{kontol.replace("[Ahmad-CP] ", "")}{N}')
                try:
                    log_hasil(titid[0].replace("[Ahmad-CP] ", ""), titid[1])
...
def file():
            logo()
            print('')
            print("\033[96;1m[1] File Cloning ")
            key = input("\n [+] Select One : ")
            if key in [""]:
...
# PROSES CRACK METODE 3 in 1
    def __metode__(self, cebok, user, pasw):
        global ok,cp,loop
        animasi = random.choice(["\x1b[1;92m[Ahmad]","\x1b[1;92m[Ahmad]","\x1b[1;92m[Ahmad]","\x1b[1;92m[Ahmad]","\x1b[1;92m[Ahmad]","\x1b[1;92m[Ahmad]","\x1b[1;92m[Ahmad]"])
            ...
            for pw in pasw:
                ...
                if "c_user" in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        wrt = '[Ahmad-CP] %s • %s • %s %s %s' % (user,pw,day,month,year)
                        cp.append(wrt)
    ...
    def kombinasi_pw(self,url):
        ...
        print('%s[%s1%s] Auto Choice'%(H,H,H))
